...
async def async_lookup_tool(key: str) -> str:
    """Lookup a color asynchronously."""
    import asyncio
...
class TestRLMWithDummyLM:
    ...
    def test_with_async_tool_e2e(self):
        """Test RLM calling an async host-side tool through the sandbox."""
        with dummy_lm_context(
            [
                {
                    "reasoning": "Look up the color of apple",
                    "code": 'color = async_lookup_tool(key="apple")\nSUBMIT(color)',
                },
            ]
        ):
            rlm = RLM("fruit -> color: str", max_iterations=3, tools=[async_lookup_tool])
            result = rlm.forward(fruit="apple")
    ...
    @pytest.mark.asyncio
    async def test_aforward_with_wrapped_async_tool_e2e(self):
        """Test aforward() with an explicitly wrapped async DSPy Tool."""
        with dummy_lm_context(
            [
                {"reasoning": "Look up the color of apple", "code": 'color = async_lookup(key="apple")\nSUBMIT(color)'},
            ]
        ):
            wrapped_tool = Tool(async_lookup_tool, name="async_lookup")
            rlm = RLM("fruit -> color: str", max_iterations=3, tools=[wrapped_tool])
            result = await rlm.aforward(fruit="apple")
